...
bool QLProofResult::Verify(const Hash digest) {
  ...
  for (size_t i = 0; i < proof.size(); ++i) {
    if (proof[i].empty()) {
      std::unique_ptr<byte_t[]> node(new byte_t[Hash::kByteLength]);
      memcpy(node.get(), doc_hash.value(), Hash::kByteLength);
      doc_hash = Hash::ComputeFrom(node.get(), Hash::kByteLength);
    } else {
      std::unique_ptr<byte_t[]> node(new byte_t[Hash::kByteLength * 2]);
      ...
      if (pos[i] == 0) {
        ...
        memcpy(node.get(), doc_hash.value(), Hash::kByteLength);
        memcpy(node.get() + Hash::kByteLength, h.value(), Hash::kByteLength);
      }
      doc_hash = Hash::ComputeFrom(node.get(), Hash::kByteLength * 2);
    }
  }
...
void QLDB::CreateLedger(const std::string& name) { db_.Put(name, "0"); }

Slice QLDB::GetData(const std::string& name, const std::string& key) const {
...
bool QLDB::Set(const std::string& name, const std::vector<std::string>& keys,
               const std::vector<std::string>& vals) {
  if (keys.size() != vals.size()) {
    std::cerr << "[QLDB] keys and vals size not equal" << std::endl;
    return false;
  } else if (keys.size() == 0) {
    return true;
  }
  ...
  // create block
  auto block = QLBlock::Encode({Slice(name), seqno}, {0, now}, now, block_hash,
                               prev_hash, documents);
  ...
  while (last_seqno > 0) {
    ...
    if (pr_last_idx == 0) {
      ...
      if (level == 1) {
        ...
        std::string prev_key = "proof_" + name + "|" +
...
QLProofResult QLDB::getProof(const std::string& name, const uint64_t tip,
  ...
  while (doc_size > 0) {
    ...
    if (curr_seq == doc_size && doc_size % 2 == 0) {
      ...
      // std::cerr << level << ", " << pr_seq << ": " << Hash(node.data()) << ",
      // " << Hash(node.data() + Hash::kByteLength) << std::endl;
      result.proof.emplace_back(hash.ToBase32());
    }
    curr_seq = pr_seq;
    doc_size /= 2;
  ...
  while (latest > 0) {
    ...
    if (curr_seq == latest && latest % 2 == 0) {
      ...
      db_.Get(key, &nodestr);
      Slice node(nodestr);
      // std::cerr << "block_" << level << ", " << pr_seq << ": " <<
      // Hash(node.data()) << ", " << Hash(node.data() + Hash::kByteLength) <<
      // std::endl;
