...
void execNewOrder(std::vector<std::string>& keys, mockstrongstore::Client* client) {
  auto w_id = keys[0];
  auto d_id = keys[1];
  auto c_id = keys[2];
...
void execPayment(std::vector<std::string>& keys, mockstrongstore::Client* client) {
  auto w_id = keys[0];
  auto d_id = keys[1];
  auto c_id = keys[2];
  auto payment = std::stod(keys[3]);
  std::cout << "BufferKey" << std::endl;
  client->BufferKey("w_" + w_id);
  client->BufferKey("d_" + d_id + "_" + w_id);
  client->BufferKey("c_" + c_id + "_" + d_id + "_" + w_id);
  std::map<string, string> values;
  std::cout << "BatchGet" << std::endl;
  client->BatchGet(values);
...
void execOrderStatus(std::vector<std::string>& keys, mockstrongstore::Client* client) {
  auto w_id = keys[0];
  auto d_id = keys[1];
  auto c_id = keys[2];
  client->BufferKey("w_" + w_id);
...
void execDelivery(std::vector<std::string>& keys, mockstrongstore::Client* client) {
  auto w_id = keys[0];
...
void execStockLevel(std::vector<std::string>& keys, mockstrongstore::Client* client) {
  auto w_id = keys[0];
  auto d_id = keys[1];
...
txnThread(Mode mode, strongstore::Mode strongmode, const char *configPath,
  ...
  mockstrongstore::Client *client = new mockstrongstore::Client(
    0, "/home/xinyu.chen/LedgerDatabase/logs/tpcc"); // mock
