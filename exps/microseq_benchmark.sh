...
for n_acc in "${load_account[@]}"; do
    for batch_size in "${batch_sizes[@]}"; do
        for value_size in "${value_sizes[@]}"; do
            ...
            # 运行测试并提取结果
            ../build_release_${db_name}/bin/microseqBenchmark -a $n_acc -b $batch_size -t $num_transaction_version -z $batch_size -k $key_size -v $value_size -d $data_path -r $result_path
            sleep 5
            set +x
