...
#include "mocks/strongstore/client.h"  // mock
...
std::string default_val[10] = {"aaaaaaaaaaaaaaaaaaaa", "bbbbbbbbbbbbbbbbbbbb",
...
int taskGenerator(int clientid, int tid, int tlen, int wPer, int rPer,
                  int timeout) {
  timeval t0;
...
int verifyThread(mockstrongstore::Client *client, int idx, int tLen, int wPer,
                 int duration, size_t timeout) {
  while (1) {
    millisleep(timeout);
    if (!running) {
    ...
    fprintf(stderr, "%ld %ld.%06ld %ld.%06ld %ld %d %d\n",
            unverified_keys.size(), t0.tv_sec, t0.tv_usec, t1.tv_sec,
            t1.tv_usec, latency, vs ? 1 : 0, 3);
...
int txnThread(mockstrongstore::Client *client, int idx, int tLen, int wPer,
              int rPer, int duration) {
      ...
      for (auto &replicas : unverified_keys) {
        if (verifymap.find(replicas.first) != verifymap.end()) {
          for (auto &blocks : replicas.second) {
            if (verifymap[replicas.first].find(blocks.first) !=
                verifymap[replicas.first].end()) {
...
int main(int argc, char **argv) {
  ...
  while ((opt = getopt(argc, argv, "c:d:N:l:w:g:k:f:m:e:s:z:r:i:t:x:")) != -1) {
    switch (opt) {
      ...
      case 'x':  // transaction rate
      {
        ...
        if ((*optarg == '\0') || (*strtolPtr != '\0') || (txn_rate < 0)) {
  ...
  mockstrongstore::Client *client =
      new mockstrongstore::Client(timeout);  // mock

  std::vector<std::future<int>> actual_ops;
  int numThread = 10;
  int interval = 1000 / txn_rate;
  for (int i = 0; i < numThread; ++i) {
    actual_ops.emplace_back(std::async(std::launch::async, taskGenerator, idx,
                                       i, tLen, wPer, rPer, interval));
  }
  actual_ops.emplace_back(std::async(std::launch::async, txnThread, client, idx,
                                     tLen, wPer, rPer, duration));
#ifndef AMZQLDB
  std::cout << "start verification thread" << std::endl;
  actual_ops.emplace_back(std::async(std::launch::async, verifyThread, client,
                                     idx, tLen, wPer, duration, timeout));
#endif
}
